...
class MainActivity : AppCompatActivity() {
        ...
        userRef.get()
            .addOnSuccessListener { document ->
                if (!document.exists()) {
                    ...
                    var listaContactos = mutableListOf(email)

                    userRef.set(
                        mapOf(
                            "user" to email,
                            "register-date" to FieldValue.serverTimestamp(),
                            "name" to nombre,
                            "lastname" to apellidos,
                            "birth-date" to fechaNac,
                            "connections" to listaContactos
                        )
                    )
    ...
    // Registrar el launcher para el registro de cuenta
